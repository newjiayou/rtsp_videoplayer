...
private:
    ...
    void StartPacingThread();
    void StopPacingThread();
    void EnqueuePacedPacket(MediaChannelId channel_id, RtpPacket pkt);
    void PacingLoop();
    int  SendPacketNow(MediaChannelId channel_id, RtpPacket pkt);
    ...
    std::deque<QueuedRtpPacket> pacing_queue_;
    std::thread pacing_thread_;
    std::atomic_bool pacing_stop_{false};
    std::chrono::steady_clock::time_point pacing_next_send_time_;
};
